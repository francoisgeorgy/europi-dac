...
class Scope(EuroPiScript):
    ...
    def main(self):
        ...
        while True:
            ...
            # a_voltage = a0._sample_adc(1)
            a1 = 65535 - adc1.read_u16()
            a2 = 65535 - adc2.read_u16()
            print(f"{a1:5}  {a2:5}")

            oled.scroll(-1, 0)
            oled.vline(RIGHT_EDGE, 0, OLED_HEIGHT, 0)

            y1 = self.calc_y_pos(max_disp_voltage, a1)
            y2 = self.calc_y_pos(max_disp_voltage, a2)

            oled.pixel(RIGHT_EDGE, y1, 1)
            c2 = (c2 + 1) % c2_dash
            oled.pixel(RIGHT_EDGE, y2, 1 if c2 > 1 else 0)
